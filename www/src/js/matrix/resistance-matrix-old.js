  ...
  */
   ...
   */
			...
			value: function _createSVG() {
				var svg = d3.select(this._container).append('svg');

				svg.append('defs').append('g').attr('id', 'cell-circle').append('circle').style('fill', 'inherit').style('transform', 'translate(40px)').style('transition', 'transform 2s').style('stroke', '#000').style('stroke-width', 1)
   ...
   */
			...
			value: function _drawCell(row, scale) {
				...
				var rowCells = row.selectAll('.cell')
				...
				var gs = rowCells.enter().call(function (d) {
				...
				.attr('data-color', function (d) {
					...
					var x = self._getMaxRowLabelWidth() + self._configuration.spaceBetweenLabelsAndMatrix + i * scale.step();
   ...
   */
			...
			value: function _drawCellRectangle(cells, scale) {
				...
				cells.append('use').attr('xlink:href', '#cell-circle').style('fill', function (d) {
...
//# sourceMappingURL=resistance-matrix-old.es2015.js.map
